...
실제 GPS 기반 병원 목록은 `GET /api/hospitals`에서 `provider=auto`로 동작합니다.

검색 우선순위는 다음과 같습니다.

1. 네이버 지역검색 API + 네이버 지도 Geocoding/Directions API
2. 카카오 Local API
3. HIRA 병원정보 API

배포 서버(Render/Railway) 환경변수에 아래 값 중 하나 이상을 설정해야 실제 병원 검색이 동작합니다.
...
API 키는 GitHub 소스코드에 직접 넣지 말고 배포 서버 환경변수에만 등록해야 합니다.
